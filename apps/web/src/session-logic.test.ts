...
describe("deriveTimelineEntries", () => {
  ...
  it("moves a queued user message to just before its turn's first entry", () => {
    const entries = deriveTimelineEntries(
      [
        {
        ...
        },
        {
          ...
          role: "user",
          text: "second request, queued while turn-a was running",
          createdAt: "2026-02-23T00:00:02.000Z",
  ...
  it("leaves a user message in createdAt order when its turn has no entries yet", () => {
    const entries = deriveTimelineEntries(
      [
        {
          id: MessageId.make("message-1"),
    ...
    expect(entries.map((entry) => entry.id)).toEqual(["message-1", "message-x"]);
  });
});
