...
describe("deriveWorkLogEntries quiet-timeline guarantee", () => {
  it("concurrent subagents replace their launch tools with one lifecycle row", () => {
    ...
    for (let agent = 0; agent < 5; agent += 1) {
      ...
      expect(deriveWorkLogEntries(activities)).toHaveLength(agent);
      activities.push(
        makeActivity({
          id: `started-${agent}`,
          kind: "task.started",
      ...
      // One row per direct spawn; the timeline batches them per segment.
      const runningEntries = deriveWorkLogEntries(activities);
      expect(runningEntries).toHaveLength(agent + 1);
      expect(runningEntries[agent]!.id).toBe(`started-${agent}`);
      expect(runningEntries[agent]!.agentSpawn?.agentTaskIds).toEqual([taskId]);
