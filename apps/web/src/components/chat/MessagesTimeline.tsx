...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  // place; the next turn (or a reload, since this is local state) folds it. A
  // fold spans a whole segment, so its id only comes from the derived rows.
  useEffect(() => {
    ...
    const foldIdOfTurn = (turnId: TurnId) =>
      rawRows.find((row) => row.kind === "turn-fold" && row.turnId === turnId)?.id;
    if (latestTurn.turnId === previous.turnId) {
      const interruptedFoldId =
        previous.state === "running" && latestTurn.state === "interrupted"
          ? foldIdOfTurn(latestTurn.turnId)
          : undefined;
      if (interruptedFoldId !== undefined) {
        setExpandedFoldIds((existing) => new Set(existing).add(interruptedFoldId));
      }
      return;
    }
    const previousFoldId = foldIdOfTurn(previous.turnId);
    if (previousFoldId === undefined) {
      return;
    }
    setExpandedFoldIds((existing) => {
      if (!existing.has(previousFoldId)) {
        return existing;
      }
      const next = new Set(existing);
