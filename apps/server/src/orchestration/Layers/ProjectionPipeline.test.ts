...
it.effect("rebinds the claimed user message to the running turn's id", () =>
  Effect.gen(function* () {
    const { dbPath } = yield* ServerConfig;
    const persistenceLayer = makeSqlitePersistenceLive(dbPath);
    ...
    const threadId = ThreadId.make("thread-claim-message");
    const turnId = TurnId.make("turn-claim-message");
    const messageId = MessageId.make("message-claim-message");
    const messageSentAt = "2026-02-26T15:00:00.000Z";
    const turnStartedAt = "2026-02-26T15:00:01.000Z";
    ...
    const rows = yield* Effect.gen(function* () {
      ...
      yield* eventStore.append({
        type: "thread.message-sent",
        eventId: EventId.make("evt-claim-message-1"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: messageSentAt,
        commandId: CommandId.make("cmd-claim-message-1"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-claim-message-1"),
        metadata: {},
        payload: {
          threadId,
          messageId,
      ...
      yield* eventStore.append({
        type: "thread.turn-start-requested",
        eventId: EventId.make("evt-claim-message-2"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: turnStartedAt,
        commandId: CommandId.make("cmd-claim-message-2"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-claim-message-2"),
        metadata: {},
        payload: {
          threadId,
          messageId,
      ...
      yield* eventStore.append({
        type: "thread.session-set",
        eventId: EventId.make("evt-claim-message-3"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: sessionSetAt,
        commandId: CommandId.make("cmd-claim-message-3"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-claim-message-3"),
        metadata: {},
        payload: {
          threadId,
          session: {
      ...
      return { messageRows, turnRows };
    }).pipe(Effect.provide(projectionLayer));

    assert.deepEqual(rows.messageRows, [
      {
        messageId: "message-claim-message",
        turnId: "turn-claim-message",
      },
    ]);
    assert.deepEqual(rows.turnRows, [
      {
        turnId: "turn-claim-message",
        pendingMessageId: "message-claim-message",
      },
    ]);
  }).pipe(
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "t3-projection-pipeline-claim-message-",
        }),
        NodeServices.layer,
      ),
    ),
