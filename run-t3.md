...
- No login, behind a switch. With `T3CODE_UNSAFE_NO_AUTH=1` (set by `run-t3.sh`), requests without a credential, or with a rejected one, get a full-scope session (`unsafe-no-auth`) instead of a 401, and websocket tickets issued to that session are accepted. Without the variable upstream auth applies unchanged, so upstream's auth tests stay as they are. Files: `apps/server/src/auth/EnvironmentAuth.ts` (one added test in `EnvironmentAuth.test.ts`).
...
- Light by default. Without a stored theme preference the chat starts light instead of following the OS appearance. Files: `apps/web/index.html` (boot script), `apps/web/src/hooks/useTheme.ts` (`DEFAULT_THEME_SNAPSHOT`), with the mirrored tests.
